...
void ESP()
{
    ...
    for (int i = 0; i < T.Count(); i++)
        ImGui::GetBackgroundDrawList()->AddText(nullptr, 16, ImVec2(10, 10 + (i * 30)), ImColor(128, 0, 0),
...
void entitiesESP()
{
    ...
    if (T.IsValid())
    {
        for (int i = 0; i < T.Count(); i++) //NUM = 11
        {
            Config.GetPalPlayerCharacter()->GetPalPlayerController()->ProjectWorldLocationToScreen(
                SDK::FVector(T[i]->K2_GetActorLocation().X, T[i]->K2_GetActorLocation().Y,
                             T[i]->K2_GetActorLocation().Z), &pokemonCOORD, true);
            ImGui::GetBackgroundDrawList()->AddText(nullptr, 16, ImVec2(pokemonCOORD.X, pokemonCOORD.Y),
                                                    ImColor(255, 255, 255), T[i]->GetName().c_str());
        }
    }
}
...
void DrawUActorComponent(SDK::TArray<SDK::UActorComponent*> Comps, ImColor color)
{
    ...
    if (Comps.IsValid())
    {
        for (int i = 0; i < Comps.Count(); i++)
        {
            if (Comps[i] != NULL)
            {
                ImGui::GetBackgroundDrawList()->AddText(nullptr, 16, ImVec2(10, 10 + (i * 30)), color,
